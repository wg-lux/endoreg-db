...
def export2dfs(
    ...
    patient_df = pd.DataFrame([patient.model_dump() for patient in patient_data_list])

    readout_df = pd.DataFrame([readout.model_dump() for readout in readout_data_list])

    lab_df = pd.DataFrame([lab.model_dump() for lab in lab_data_list])

    fu_tx_df = pd.DataFrame([fu_tx.model_dump() for fu_tx in fu_tx_data_list])
    ...
    for patient in patient_data_list:
        ...
        for transplant_id in patient.transplant_ids:
            _entry = {"patient_id_ntx": patient_id_ntx, "transplant_id": transplant_id}
    ...
    for patient in patient_data_list:
        ...
        for case_id_ukw in patient.case_ids_ukw:
            _entry = {"patient_id_ntx": patient_id_ntx, "case_id_ukw": case_id_ukw}
