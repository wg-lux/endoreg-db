...
def sync_patient_readout_data(
    ...
    for readout_data in readout_data_list:
        ...
        if patient_hash in patient_data_by_hash:
            ...
            readout_data.patient_id_ntx = patient_data_by_hash[
                patient_hash
            ].patient_id_ntx
        else:
...
def sync_patient_lab_data(
    patient_data_list: List[PatientData], lab_data_list: List[LabData]
...
def patient_df_etl(
    readout_data_list: List[ReadoutData], lab_data_list: List[LabData]
    ...
    patient_data_list, lab_data_list, unmatched = sync_patient_lab_data(
        patient_data_list=patient_data_list, lab_data_list=lab_data_list
    )
    ...
    for lab in unmatched[:5]:
        _str += (
            f"  patient_id_ukw: {lab.patient_id_ukw}, case_id_ukw: {lab.case_id_ukw}\n"
        )
    ic(_str)
