...
def _create_video(tmp_path: Path) -> VideoFile:
    ...
    video = VideoFile.objects.create(
        center=center,
        raw_video_hash=f"temporal-video-{uuid.uuid4().hex}",
        frame_count=4,
        duration=0.16,
...
def test_run_video_temporal_inference_materializes_lx_core_segments(
    ...
    monkeypatch.setattr(
        jobs, "extract_video_frames", _extract_video_frames_success, raising=False
    )
    monkeypatch.setattr(
        jobs,
    ...
    monkeypatch.setattr(
        ...
        _has_extracted_frame_files_true,
        raising=False,
    )
...
def test_run_video_temporal_inference_stream_succeeds_when_extract_frames_would_fail(
    ...
    monkeypatch.setattr(jobs, "update_video_meta", _update_video_meta_noop)
    monkeypatch.setattr(
        jobs, "extract_video_frames", _extract_video_frames_forbidden, raising=False
    )
    monkeypatch.setattr(
        jobs,
        "update_video_text_metadata",
...
def test_temporal_inference_rejects_retired_frame_cache() -> None:
    with pytest.raises(ValueError, match="Frame file caches are retired"):
        jobs._resolve_temporal_frame_source_mode(VideoFile(), "cache")
