...
class UploadEndpointTests(TestCase):
    ...
    def test_local_study_server_rejects_unauthenticated_upload(self):
        uploaded = SimpleUploadedFile(
            name="upload-test.pdf",
            content=MINIMAL_PDF_BYTES,
            content_type="application/pdf",
    ...
    @override_settings(ENDOREG_DEPLOYMENT_ROLE="local_study_server")
    def test_local_study_server_requires_declared_center_key(self):
        user = User.objects.create_user(username="study-user", password="secret")
        self.client.force_login(user)
        uploaded = SimpleUploadedFile(
            name="upload-test.pdf",
            content=MINIMAL_PDF_BYTES,
    ...
    @override_settings(ENDOREG_DEPLOYMENT_ROLE="local_study_server")
    def test_local_study_server_rejects_center_mismatch(self):
        center_a = Center.objects.create(name="study-a", display_name="Study A")
        center_b = Center.objects.create(name="study-b", display_name="Study B")
        user = User.objects.create_user(username="study-user", password="secret")
        examiner = Examiner.objects.create(
            first_name="Study",
            last_name="User",
            hash="study-user-hash",
            center=center_a,
        )
        ...
        with (
            ...
            response = self.client.post(
                "/api/upload/",
                data={"file": uploaded, "center_key": center_b.center_key},
            )

        assert response.status_code == 403, response.content
        assert "outside the authenticated scope" in response.json()["error"]

    @override_settings(ENDOREG_DEPLOYMENT_ROLE="local_study_server")
    def test_local_study_server_accepts_authenticated_center_scoped_upload(self):
        center = Center.objects.create(name="study-center", display_name="Study Center")
        user = User.objects.create_user(username="study-user", password="secret")
        examiner = Examiner.objects.create(
            first_name="Study",
            last_name="User",
