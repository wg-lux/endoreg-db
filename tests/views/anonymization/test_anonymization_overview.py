...
def test_anonymization_overview_mixed_content():
    ...
    dummy_video = SimpleUploadedFile("old_video.mp4", b"dummy_content", content_type="video/mp4")
    ...
    # Force older date
    VideoFile.objects.filter(pk=video.pk).update(uploaded_at=timezone.now() - timezone.timedelta(days=1))
    ...
    dummy_pdf = SimpleUploadedFile("new_report.pdf", b"dummy_pdf_content", content_type="application/pdf")
    ...
    # Verify Statuses
    assert data[0]["anonymizationStatus"] == AnonymizationState.DONE_PROCESSING_ANONYMIZATION
