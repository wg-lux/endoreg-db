...
class AnonymizationOverviewAPITest(TestCase):
    ...
    def test_video_sm_creation(self):
        ...
        if SKIP_EXPENSIVE_TESTS:
            self.skipTest(
                "Skipping video SensitiveMeta test (requires real video file)"
            )
        ...
        self.assertIsInstance(
            video_sm,
            SensitiveMeta,
            "SensitiveMeta should be an instance of SensitiveMeta model",
        )
        self.assertIsInstance(
            video_sm.state, SensitiveMetaState, "SensitiveMeta should have a state"
        )
        self.assertIsInstance(
            video.state, VideoState, "VideoFile should have a VideoState"
        )
        self.assertEqual(
            video_sm.center,
            self.center,
            "SensitiveMeta should be linked to the correct center",
        )
    ...
    def test_pdf_sm_creation(self):
        ...
        self.assertIsInstance(
            pdf_sm.state, SensitiveMetaState, "SensitiveMeta should have a state"
        )
        self.assertEqual(
            pdf_sm.center,
            self.center,
    ...
    def test_overview_mixed_files(self):
        ...
        self.assertIsNotNone(
            video.state.frames_extracted, "Frames should be extracted for the video"
        )

        # SIMULATE ANONYMIZATION
        video.mark_sensitive_meta_processed()
        self.assertTrue(
            video.state.sensitive_meta_processed,
            "Sensitive meta should be marked as processed",
        )

        video.mark_sensitive_meta_verified()
        self.assertTrue(
            video.sensitive_meta.state.dob_verified,
            "Sensitive meta (dob) should be marked as verified",
        )
        self.assertTrue(
            video.sensitive_meta.state.names_verified,
            "Sensitive meta (names) should be marked as verified",
        )
        video.anonymize()
        ...
        pdf_item = data_sorted[
            1
        ]  # During setup, pdf gets called before video and therefore is older
        ...
        # self.assertEqual(pdf_item['anonymizationStatus'], 'done_processing_anonymization')
        # TODO
        ...
        self.assertEqual(
            video_item["anonymizationStatus"], "done_processing_anonymization"
        )
        ## self.assertEqual(video_item['annotationStatus'], 'done_processing_anonymization')  # Sensitive meta is verified
