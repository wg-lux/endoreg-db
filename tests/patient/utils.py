...
from .conf import TEST_CENTER_NAME, TEST_EXAMINATION_NAME_STRINGS
...
def create_patient_with_colonoscopy(
    center_name: str = TEST_CENTER_NAME,
) -> Tuple[Patient, PatientExamination]:
    patient = Patient.create_generic(center=center_name)

    patient_examination = patient.create_examination(examination_name_str="colonoscopy")

    return patient, patient_examination


def create_patient_with_endoscopies(
    center_name: str = TEST_CENTER_NAME,
) -> Tuple[Patient, List[PatientExamination]]:
    patient = Patient.create_generic(center=center_name)
    ...
    for examination_name_str in TEST_EXAMINATION_NAME_STRINGS:
        ...
        patient_examination = patient.create_examination(
            examination_name_str=examination_name_str, date_start=start_date
        )
