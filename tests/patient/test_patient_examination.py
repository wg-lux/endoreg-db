...
from endoreg_db.models import Center, ExaminationIndication, Patient, PatientExamination
...
from .utils import create_patient_with_endoscopies
...
class TestGeneratePatient(TestCase):
    ...
    def test_create_endoscopies(self):
        ...
        patient, patient_examinations = create_patient_with_endoscopies(
        ...
        for examination_name_str in examination_name_strings:
            patient_examination = PatientExamination.objects.get(
                patient=patient, examination__name=examination_name_str
            )
    ...
    def test_examination_indication(self):
        ...
        for examination_indication in examination_indications:
            for examination_indication_choice in examination_indication.get_choices():
                (
                    patient_examination,
                    patient_examination_indication,
                ) = patient.create_examination_by_indication(
                ...
                patient_examination_indication.indication_choice = (
                    examination_indication_choice
                )
        ...
        with open(TEST_PATIENT_EXAMINATION_INDICATION_OUTPUT_PATH, "w") as f:
            for patient_examination in patient_examinations:
                ...
                examination_indication_choices = (
                    patient_examination.get_indication_choices()
                )
                assert len(examination_indication_choices) == 1
                examination_indication_choice = examination_indication_choices[0]

                f.write(
                    f"Examination Indication Choice: {examination_indication_choice}\n\n"
                )
