    ...
    for path in sorted(directory.glob("*.yaml")):
        ...
        if not isinstance(content, list):
            raise AssertionError(
                f"Expected a list of entries in {path}, got {type(content)!r}"
            )
        for entry in content:
            if not isinstance(entry, dict):
                raise AssertionError(
                    f"Expected each entry to be a mapping in {path}, got {type(entry)!r}"
                )
            yield path, entry
...
def _gather_requirement_fields() -> List[Tuple[Path, dict]]:
    ...
    for path, entry in _load_yaml_entries(REQUIREMENT_DIR):
        ...
        if not isinstance(fields, dict):
            raise AssertionError(
                f"Requirement entry in {path} is missing a 'fields' mapping"
            )
...
def _gather_operator_names() -> set[str]:
    ...
    for path, entry in _load_yaml_entries(OPERATOR_DIR):
        ...
        if not isinstance(fields, dict):
            raise AssertionError(
                f"Requirement operator entry in {path} is missing a 'fields' mapping"
            )
        name = fields.get("name")
        if not isinstance(name, str) or not name:
            raise AssertionError(
                f"Requirement operator entry in {path} is missing a valid name"
            )
...
REQUIREMENT_IDS = [
    f"{path.name}:{fields.get('name', '<unnamed>')}"
    for path, fields in REQUIREMENT_FIELDS
]
OPERATOR_NAMES = _gather_operator_names()


@pytest.mark.parametrize("path, fields", REQUIREMENT_FIELDS, ids=REQUIREMENT_IDS)
def test_requirement_operators_reference_existing_entries(
    path: Path, fields: dict
) -> None:
    missing = [
        operator
        for operator in fields.get("operators", [])
        if operator not in OPERATOR_NAMES
    ]
    assert not missing, (
        f"Requirement '{fields.get('name')}' in {path.name} references unknown operators: {', '.join(missing)}"
    )
