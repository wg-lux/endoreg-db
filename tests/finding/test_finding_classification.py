...
class ColonoscopyFindingTest(TestCase):
    ...
    def setUp(self):
        ...
        self.bbps_simple_classification = FindingClassification.objects.get(
            name=BBPS_SIMPLE_CLASSIFICATION_NAME
        )
        self.bbps_simple_choice = FindingClassificationChoice.objects.get(
        ...
        self.patient_examination: PatientExamination = self.patient.create_examination(
            examination_name_str=EXAMINATION_NAME, save=True
        )

        self.patient_finding_bp: PatientFinding = (
            self.patient_examination.create_finding(self.colo_finding_bp)
        )
    ...
    def test_colonoscopy_bbps_simple_choices(self):
        ...
        self.assertIsNotNone(
            self.colo_finding_bp,
            "Colonoscopy finding with name 'colo_finding_bp_simple' not found.",
        )

        # Make sure that the finding is linked to the colonoscopy examination
        self.assertIn(
            self.colo_finding_bp,
            self.colonoscopy.findings.all(),
            "Colonoscopy should have 'bowel_preparation_simplified' finding associated with it.",
        )
