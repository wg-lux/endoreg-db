...
from endoreg_db.models.medical.laboratory.lab_value import (
    CommonLabValues,
)  # Keep for class structure
import logging

from ..helpers.data_loader import load_data
from endoreg_db.models.other.gender import Gender
from endoreg_db.models.administration.person.patient import Patient
from endoreg_db.models.other.distribution import (
    NumericValueDistribution,
)  # For type hinting/mocking
...
class CommonLabValuesTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.platelets = LabValue.objects.get(
            name="platelets"
        )  # Used as lv_with_dist_ref
        ...
        cls.male_gender, _ = Gender.objects.get_or_create(
            name="male", defaults={"name_de": "männlich", "name_en": "male"}
        )
        cls.female_gender, _ = Gender.objects.get_or_create(
            name="female", defaults={"name_de": "weiblich", "name_en": "female"}
        )
        cls.unknown_gender, _ = Gender.objects.get_or_create(
            name="unknown", defaults={"name_de": "unbekannt", "name_en": "unknown"}
        )
    ...
    def test_common_lab_values(self):
        ...
        self.assertIsInstance(
            common_lab_values, CommonLabValues, "Expected CommonLabValues instance."
        )
    ...
    def test_get_increased_value_no_distribution_with_upper_bound(self):
        ...
        try:
            ...
            self.assertEqual(
                increased_value, 350 + (350 * self.platelets.bound_adjustment_factor)
            )
        finally:
    ...
    def test_get_increased_value_no_distribution_no_upper_bound(self):
        ...
        try:
            with self.assertWarnsRegex(
                UserWarning,
                "Cannot determine an increased value.*without.*an upper normal range",
            ):
    ...
    def test_get_normal_value_no_distribution_no_bounds(self):
        ...
        try:
            with self.assertWarnsRegex(
                UserWarning,
                r"Cannot determine a normal value.*without a numerical distribution or a normal range",
            ):
    ...
    def test_get_decreased_value_no_distribution_with_lower_bound(self):
        ...
        try:
            ...
            self.assertEqual(
                decreased_value, 150 - (150 * self.platelets.bound_adjustment_factor)
            )
        finally:
            self.platelets.default_numerical_value_distribution = original
    ...
    def test_get_decreased_value_no_distribution_no_lower_bound(self):
        ...
        try:
            with self.assertWarnsRegex(
                UserWarning,
                "Cannot determine a decreased value.*without.*a lower normal range",
            ):
    ...
    def test_get_increased_value_with_distribution_patient(self):
        ...
        with patch.object(
            ...
            increased_value = self.lv_with_dist_ref.get_increased_value(
                patient=self.mock_male_patient
            )
            self.assertEqual(increased_value, 400)
            mock_distribution.generate_value.assert_called_once_with(
                lab_value=self.lv_with_dist_ref, patient=self.mock_male_patient
            )
            ...
            self.assertEqual(
                increased_value,
                350 + (350 * self.lv_with_dist_ref.bound_adjustment_factor),
            )
        ...
        with patch.object(
            type(self.lv_min_only_dist),
            "default_numerical_value_distribution",
            new_callable=PropertyMock,
        ) as mock_prop:
            mock_prop.return_value = mock_dist_mean_std
            mock_dist_mean_std.generate_value.return_value = 26  # > mean + stddev (25)
            increased_value = self.lv_min_only_dist.get_increased_value(
                patient=self.mock_male_patient
            )
            self.assertEqual(increased_value, 26)
            mock_dist_mean_std.generate_value.assert_called_once_with(
                lab_value=self.lv_min_only_dist, patient=self.mock_male_patient
            )
    ...
    def test_get_increased_value_with_distribution_no_patient(self):
        ...
        self.assertIsNotNone(
            self.lv_with_dist_ref.default_numerical_value_distribution,
            "Platelets (lv_with_dist_ref) must have a distribution from load_data()",
        )
        with self.assertWarnsRegex(
            UserWarning, "Cannot use numerical distribution.*without patient context"
        ):
            increased_value = self.lv_with_dist_ref.get_increased_value()
            self.assertEqual(
                increased_value,
    ...
    def test_get_normal_value_with_distribution_patient(self):
        ...
        with patch.object(
            type(self.lv_no_range_dist),
            "default_numerical_value_distribution",
            new_callable=PropertyMock,
        ) as mock_prop:
            mock_prop.return_value = mock_no_range_dist
            mock_no_range_dist.generate_value.return_value = 77
            normal_value = self.lv_no_range_dist.get_normal_value(
                patient=self.mock_male_patient
            )
            self.assertEqual(normal_value, 77)
            mock_no_range_dist.generate_value.assert_called_once_with(
                lab_value=self.lv_no_range_dist, patient=self.mock_male_patient
            )
    ...
    def test_get_decreased_value_with_distribution_patient(self):
        ...
        with patch.object(dist, "generate_value", return_value=200) as mock_gen:
            ...
            self.assertEqual(
                decreased_value,
                150 - (150 * self.lv_with_dist_ref.bound_adjustment_factor),
            )
        ...
        with patch.object(
            ...
            mock_prop.return_value = mock_dist_mean_std
            with patch.object(
                mock_dist_mean_std, "generate_value", return_value=65
            ) as mock_gen:
                decreased_value = self.lv_max_only_dist.get_decreased_value(
                    patient=self.mock_male_patient
                )
                self.assertEqual(decreased_value, 65)
                mock_gen.assert_called_once_with(
                    lab_value=self.lv_max_only_dist, patient=self.mock_male_patient
                )
            with patch.object(
                mock_dist_mean_std, "generate_value", return_value=75
            ) as mock_gen:
                decreased_value = self.lv_max_only_dist.get_decreased_value(
                    patient=self.mock_male_patient
                )
                self.assertEqual(
                    decreased_value, 75
                )  # Fallback to last generated value
