...
class ExaminationTest(TestCase):
    def setUp(self):
        ...
        self.examination_colonoscopy = Examination.objects.get(
            name=EXAMINATION_COLONOSCOPY_NAME
        )
    ...
    def test_examination_colonoscopy(self):
        ...
        self.assertEqual(
            colonoscopy.name,
            EXAMINATION_COLONOSCOPY_NAME,
            "Colonoscopy examination name should match the expected value",
        )
        self.assertTrue(
            colonoscopy.findings.exists(),
            "Colonoscopy examination should have associated findings",
        )

        polyp = colonoscopy.findings.get(name=FINDING_COLON_POLYP_NAME)
        self.assertIsNotNone(
            polyp, "Colonoscopy should have a finding named 'colon_polyp'"
        )
        self.assertEqual(
            polyp.name,
            FINDING_COLON_POLYP_NAME,
            "Finding name should match the expected value",
        )
        self.assertIn(
            self.finding_colon_polyp,
            colonoscopy.findings.all(),
            "Colonoscopy should include the 'colon_polyp' finding in its findings list",
        )
    ...
    def test_examination_serializer(self):
        ...
        self.assertIn(
            FINDING_COLON_POLYP_NAME,
            serialized_finding_names,
            "Serialized findings should include 'colon_polyp' finding",
        )
