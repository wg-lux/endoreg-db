"""
Test module for report import service functionality.

The goal is that after processing:
- An original report with file_path in /data/pdfs/sensitive
- No report should remain in /data/raw_pdfs
- report should be processed and anonymized
"""
...
class TestPdfImportFileMovement(TestCase):
    """Test report import service file movement and organization."""

    def setUp(self):
        """Set up test environment."""
        # Create minimal report file data (report header)
        self.test_pdf_data = b"%report-1.4\n1 0 obj\n<<\n/Type /Catalog\n/Pages 2 0 R\n>>\nendobj\n2 0 obj\n<<\n/Type /Pages\n/Kids [3 0 R]\n/Count 1\n>>\nendobj\n3 0 obj\n<<\n/Type /Page\n/Parent 2 0 R\n/MediaBox [0 0 612 792]\n>>\nendobj\nxref\n0 4\n0000000000 65535 f\n0000000009 00000 n\n0000000074 00000 n\n0000000120 00000 n\ntrailer\n<<\n/Size 4\n/Root 1 0 R\n>>\nstartxref\n202\n%%EOF"
        ...
        self.center = Center.objects.create(
            name="test_center", display_name="Test Center"
        )
    ...
    def create_test_pdf_file(self, filename: str = "test_document.pdf") -> Path:
        """Create a test report file in raw_pdfs directory."""
        pdf_path = self.temp_raw_pdfs / filename
        with open(pdf_path, "wb") as f:
            f.write(self.test_pdf_data)
        return pdf_path

    @patch("endoreg_db.utils.paths.REPORT_DIR")
    def test_pdf_file_movement_flow(self, mock_pdf_dir):
        """Test complete report file movement flow."""
        # Mock REPORT_DIR to use our temp directory
        ...
        self.assertTrue(test_pdf_path.exists(), "Test report file should be created")
        ...
        with patch.object(
            ...
            # Mock report creation methods
            with patch(
                "endoreg_db.models.media.pdf.raw_pdf.RawPdfFile.objects.create"
            ) as mock_create_pdf:
                mock_pdf = MagicMock()
                ...
                with patch("shutil.move"):
                    with patch("os.remove"):
                        ...
                        self.assertIsNotNone(
                            result_pdf, "report import should return a report instance"
                        )
    ...
    def test_sensitive_file_creation(self, mock_pdf_dir):
        """Test that report files are moved to sensitive directory."""
        mock_pdf_dir.__str__ = lambda: str(self.temp_pdfs)
        mock_pdf_dir.__truediv__ = lambda self, other: self.temp_pdfs / other
        ...
        with patch.object(
            ...
            with patch(
                ...
                with patch.object(
                    service, "create_sensitive_file"
                ) as mock_create_sensitive:
                    service.import_and_anonymize(
                        file_path=test_pdf_path, center_name=self.center.name
                    )
    ...
    def test_hash_based_naming(self, mock_pdf_dir):
        """Test that report files are named using their content hash."""
        mock_pdf_dir.__str__ = lambda: str(self.temp_pdfs)
        mock_pdf_dir.__truediv__ = lambda self, other: self.temp_pdfs / other
    ...
    def test_file_lock_mechanism(self):
        ...
        with service._file_lock(test_pdf_path):
            ...
            self.assertTrue(
                lock_path.exists(), "Lock file should exist during processing"
            )

        # Lock file should be cleaned up after processing
        self.assertFalse(
            lock_path.exists(), "Lock file should be cleaned up after processing"
        )
    ...
    def test_quarantine_functionality(self):
        ...
        self.assertFalse(
            test_pdf_path.exists(), "Original file should be moved to quarantine"
        )

        # File should exist in quarantine directory
        self.assertTrue(
            quarantine_path.exists(), "File should exist in quarantine directory"
        )
        self.assertTrue(
            "_processing" in str(quarantine_path),
            "Quarantine path should contain '_processing'",
        )
    ...
    def test_processing_context_management(self):
        ...
        self.assertTrue(
            expected_keys.issubset(service.processing_context.keys()),
        ...
        self.assertEqual(
            service.processing_context, {}, "Processing context should be cleaned up"
        )
