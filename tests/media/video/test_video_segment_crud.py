...
def create_test_video_segment(
    client, video, label, start_frame_number, end_frame_number
) -> Response:
    """
...
class TestLabelVideoSegmentCRUD:
    ...
    def setup_method(self):
        ...
        if SKIP_EXPENSIVE_TESTS or not RUN_VIDEO_TESTS:
            pytest.skip(
                "Segment CRUD tests require RUN_VIDEO_TESTS and SKIP_EXPENSIVE_TESTS=false"
            )
        ...
        self.label_type = LabelType.objects.create(
            name="Test Label Type", description="A test label type for video segments"
        )
    ...
    def test_create_segment_success(self):
        ...
        assert response.data["start_frame_number"] == pytest.approx(
    ...
    def test_list_segments_with_filtering(self):
        ...
        response = cast(
            Response, self.client.get(f"/api/video-segments/?video_id={self.video.pk}")
        )
        assert response.status_code == status.HTTP_200_OK
        assert response.data is not None
        assert len(response.data) == 2
    ...
    def test_update_segment_partial(self):
        ...
        response = cast(
            Response,
            self.client.patch(
                f"/api/video-segments/{segment_id}/", data, format="json"
            ),
        )
        ...
        assert response.data is not None
        assert (
            response.data["end_frame_number"] == self.end_frame_number + self.video_fps
        )
        assert (
            response.data["start_frame_number"] == self.start_frame_number
        )  # Unverändert
    ...
    def test_update_segment_full(self):
        ...
        response = cast(
            Response,
            self.client.put(f"/api/video-segments/{segment.pk}/", data, format="json"),
        )
    ...
    def test_create_segment_validation_error(self):
        ...
        assert any(
            "end_frame_number must be greater than start_frame_number" in str(msg)
