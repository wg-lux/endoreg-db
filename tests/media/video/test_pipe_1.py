...
def _test_pipe_1(test: "VideoFileModelExtractedTest"):
    ...
    if isinstance(video_file, VideoFile):
        ...
        test.assertIsNotNone(
            video_file.video_meta, "VideoMeta should exist after pipe_1"
        )
        test.assertIsNotNone(
            video_file.sensitive_meta, "SensitiveMeta should exist after pipe_1"
        )
        ...
        test.assertTrue(
            prediction_meta_exists, "VideoPredictionMeta should exist after pipe_1"
        )

    # Check State flags - these work for both real and mock objects
    test.assertTrue(
        state.text_meta_extracted, "State.text_meta_extracted should be True"
    )
    test.assertTrue(
    ...
    test.assertFalse(
        ...
        "State.frames_extracted should be False after pipe_1 (delete_frames_after=True)",
    )
    test.assertTrue(
        state.frames_initialized,
        "State.frames_initialized should be True after pipe_1 (delete_frames_after=True)",
    )
