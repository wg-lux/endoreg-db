...
class VideoSegmentationViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.gender, _ = Gender.objects.get_or_create(name="Male")
        self.center, _ = Center.objects.get_or_create(name="Test Center")
    ...
    def test_video_segments_creation_works_in_dev_mode(self):
        ...
        data = response.json()
        self.assertEqual(data['label_name'], 'outside')
...
class VideoSegmentationIntegrationTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create test dependencies
        self.center, _ = Center.objects.get_or_create(name="Integration Test Center")
        
        # Create minimal test data
        self.video = VideoFile.objects.create(
            original_file_name="integration_test.mp4",
            fps=30.0,
            center=self.center  # Add required center
        )
    ...
    def test_complete_segment_workflow_in_dev_mode(self):
        ...
        label_response = self.client.get(f'/api/videos/{self.video.id}/labels/test_label/')
        self.assertEqual(label_response.status_code, status.HTTP_200_OK)
        
        label_data = label_response.json()
        self.assertEqual(len(label_data['time_segments']), 1)
        
        # Step 3: Verify segment data structure
        segment = label_data['time_segments'][0]
        self.assertEqual(segment['start_time'], 10.0)
        self.assertEqual(segment['end_time'], 15.0)
        
        # Step 4: Test video streaming still works
        stream_response = self.client.get(f'/api/videostream/{self.video.id}/')
        # This should fail with 404 since we don't have actual video file, but not with 500
