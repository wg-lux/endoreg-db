...
from endoreg_db.models import Label, LabelType, LabelVideoSegment


def create_test_label(label_name="polyp", label_type_name="video_segmentation"):
    label_type, _ = LabelType.objects.get_or_create(
        name=label_type_name, defaults={"description": "Video segmentation labels"}
    )
...
def create_test_video_segment(
    video=None, label=None, start_frame=10, end_frame=20, prediction_meta=None
):
    if video is None:
