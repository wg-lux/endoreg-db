...
from endoreg_db.models import LxClientType

from .conf import CLIENT_TYPE_NAME_ABBREVIATION_TUPLES, TEST_LX_CLIENT_TYPE_OUTPUT_PATH
    ...
    def test_client_types_created(self):
        with open(TEST_LX_CLIENT_TYPE_OUTPUT_PATH, "a") as f:
            for client_type_name, abbreviation in CLIENT_TYPE_NAME_ABBREVIATION_TUPLES:
                ...
                self.assertTrue(
                    client_type, f"Client Type {client_type_name} not found"
                )
                self.assertTrue(
                    client_type.abbreviation == abbreviation,
                    f"Client Type {client_type_name} abbreviation not found",
                )
