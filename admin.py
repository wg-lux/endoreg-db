...
from endoreg_db.models import (
    Patient, Examination, PatientExamination, Finding,
...
from endoreg_db.forms.patient_finding_intervention_form import PatientFindingInterventionForm
...
class PatientFindingInterventionAdmin(admin.ModelAdmin):
    ...
    def get_location_choices_json(self, request):
        ...
        try:
            choices = list(FindingLocationClassificationChoice.objects.filter(
                location_classifications__id=location_id
            ).values("id", "name"))  #  Convert QuerySet to List
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path('ajax/get-location-choices/', self.admin_site.admin_view(self.get_location_choices_json), name='ajax_get_location_choices'),
        ]
