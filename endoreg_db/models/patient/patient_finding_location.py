...
class PatientFindingLocation(models.Model):
    location_classification = models.ForeignKey(
        "FindingLocationClassification",
        on_delete=models.CASCADE,
        related_name="patient_finding_locations",
    )
    location_choice = models.ForeignKey(
    ...
    def save(self, *args, **kwargs):
        if self.location_choice not in self.location_classification.choices.all():
            raise ValueError(
                "location_choice must be in location_classification.choices"
            )
