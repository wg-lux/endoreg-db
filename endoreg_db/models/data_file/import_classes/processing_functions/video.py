...
def get_videos_scheduled_for_frame_extraction():
    return RawVideoFile.objects.filter(
        state_frames_required=True, state_frames_extracted=False
    )
...
def get_videos_scheduled_for_ocr():
    return RawVideoFile.objects.filter(
        state_ocr_required=True, state_ocr_completed=False, state_frames_extracted=True
    )
...
def videos_scheduled_for_ocr_preflight():
    videos = RawVideoFile.objects.filter(
        state_ocr_required=True, state_ocr_completed=False, state_frames_extracted=False
    )
    for video in videos:
...
def perform_initial_prediction_on_video(
    video: RawVideoFile, model_path, window_size_s, min_seq_len_s
):
...
def perform_initial_prediction_on_videos(model_path, window_size_s, min_seq_len_s):
    videos = get_videos_scheduled_for_initial_prediction()
    for video in videos:
        perform_initial_prediction_on_video(
            video, model_path, window_size_s, min_seq_len_s
        )
...
def get_videos_scheduled_for_frame_deletion():
    return RawVideoFile.objects.filter(state_frames_required=False)
