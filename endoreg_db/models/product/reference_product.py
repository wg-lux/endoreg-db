...
class ReferenceProduct(models.Model):
    ...
    product_group = models.OneToOneField(
        "ProductGroup", on_delete=models.CASCADE, related_name="reference_product"
    )
    ...
    def set_emission_factors(self):
        ...
        (
            product_emission,
            product_emission_unit,
        ) = product.get_product_material_emission()
        (
            package_emission,
            package_emission_unit,
        ...
        assert (
            reference_unit == package_weight_unit
        ), "Package weight units do not match"
        assert (
            reference_unit == product_emission_unit
        ), "Product emission units do not match"
        assert (
            reference_unit == package_emission_unit
