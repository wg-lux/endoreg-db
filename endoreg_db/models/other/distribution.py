...
class NumericValueDistribution(BaseValueDistribution):
    ...
    def generate_value(self, lab_value, patient):
        ...
        if self.distribution_type == "uniform":
            ...
            assert (
                "min" in default_normal_range_dict
    ...
    def _generate_value_uniform(self, default_normal_range_dict: dict):
        ...
        _ = self.parse_value_descriptor(self.max_descriptor)
    ...
    def _generate_date_value(self):
        ...
        if self.distribution_type == "uniform":
            ...
            random_ordinal = np.clip(
                random_ordinal, self.date_min.toordinal(), self.date_max.toordinal()
            )
            return date.fromordinal(random_ordinal)
        else:
            raise ValueError("Unsupported distribution type")
    ...
    def _generate_timedelta_value(self):
        if self.distribution_type == "uniform":
            random_days = np.random.randint(
                self.timedelta_days_min, self.timedelta_days_max + 1
            )

        elif self.distribution_type == "normal":
            random_days = int(
                np.random.normal(self.timedelta_days_mean, self.timedelta_days_std_dev)
            )
            random_days = np.clip(
                random_days, self.timedelta_days_min, self.timedelta_days_max
            )
