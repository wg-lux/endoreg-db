...
def _setting_exists_with_value(name: str) -> bool:
...
def _set_str_if_missing(name: str, value: str) -> None:
    if not value:
        return
    if not _setting_exists_with_value(name):
...
def ensure_keycloak_settings() -> None:
    """
    ...
    """
    _set_str_if_missing("KEYCLOAK_BASE_URL", keycloak_settings.KEYCLOAK_BASE_URL)
    ...
    _set_bool_if_missing("OIDC_STORE_ID_TOKEN", keycloak_settings.OIDC_STORE_ID_TOKEN)
    _set_str_if_missing(
        "OIDC_LOGOUT_REDIRECT_URL",
        keycloak_settings.OIDC_LOGOUT_REDIRECT_URL,
    )

    _set_str_if_missing("OIDC_RP_SCOPES", keycloak_settings.OIDC_RP_SCOPES)
    _set_str_if_missing("OIDC_RP_SIGN_ALGO", keycloak_settings.OIDC_RP_SIGN_ALGO)
    _set_bool_if_missing("OIDC_VERIFY_SSL", keycloak_settings.OIDC_VERIFY_SSL)

    _set_str_if_missing("LOGIN_URL", keycloak_settings.LOGIN_URL)
    _set_str_if_missing("LOGIN_REDIRECT_URL", keycloak_settings.LOGIN_REDIRECT_URL)
    _set_str_if_missing("LOGOUT_REDIRECT_URL", keycloak_settings.LOGOUT_REDIRECT_URL)
