...
from .views.raw_pdf_meta_validation_views import PDFFileForMetaView, UpdateSensitiveMetaView
...
from .views.examination_views import (
    ExaminationViewSet,
    MorphologyChoiceListView,
    LocationChoiceListView,
    InterventionListView,
    InstrumentListView
)
from rest_framework import generics
from .models import Video  # Assumption: Video model exists
from .serializers.video import VideoListSerializer  # Minimal serializer
...
router.register(r'examinations', ExaminationViewSet, basename='examination')  # New endpoint
...
urlpatterns = [
    ...
    # The dropdown contains video names and their corresponding IDs, which are retrieved from the database (RawVideoFile).
    # Additionally, this route (api/videos) also fetches labels along with their names and IDs from the label table.
    ...
    # If editing is required, a form will be available for each label. This form dynamically updates when the selected label changes.
    # It will display all segments associated with the selected label, each with a delete option.
    # Below these segments, there may be a button for adding more segments.
    ...
    # It also fetches the labels from the label table.
    # For Keycloak:
    ...
    # path('api/prediction/', FetchSingleFramePredictionView.as_view(), name='fetch-single-frame-prediction'),
    ...
    # We need label name for this, not id as we do not have id in RawVideoFile table.
    ...
    path("api/video/sensitivemeta/", VideoFileForMetaView.as_view(), name="video_meta"),
    ...
    # - If there are errors, the API returns structured error messages.
    ...
    # - If errors occur, the API returns structured error messages.
    ...
    #     - Selects 5 frames that have the lowest "low_quality" prediction and are at least 2 seconds (100 frames) apart.
    # Fully configurable via constants at the top: POLYP_LABEL_NAME, CHROMO_LABEL_NAME, FPS, MIN_SEGMENT_LENGTH_SECONDS, MIN_SEQUENCE_GAP_SECONDS, etc.
    # Example URL:
    #   GET /api/video/niceclassification/
    ...
    # ---------------------------------------------------------------------------------------
    # This is for testing the timeline.
    # Note: Delete this URL and also remove the corresponding view and template when no longer needed.
    
    # These URLS fetch the data for the current examination as well as the locations and interventions.
    path('video/<int:video_id>/timeline/', video_timeline_view, name='video_timeline'),
    path("api/examinations/<int:exam_id>morphology-classification-choices/",
    MorphologyChoiceListView.as_view(),),
    path("api/examinations/<int:exam_id>location-classification-choices/",
    LocationChoiceListView.as_view(),),
    path("api/examinations/<int:exam_id>intervention-classification-choices/",
    InterventionListView.as_view(),),
    path("api/examinations/<int:exam_id>instrument-classification-choices/",
