...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        secret_path = (
            Path(options["source_node_secret_file"])
            .expanduser()
            .resolve()
        )
        ...
        node_secret = secret_path.read_text(
            encoding="utf-8"
        ).strip()
    
        if not node_secret:
            raise CommandError(
                f"Secret file is empty: {secret_path}"
            )
        ...
        client_certificate_path = (
            Path(options["client_certificate_file"])
            .expanduser()
            .resolve()
