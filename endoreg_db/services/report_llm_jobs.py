...
def _config_from_payload(
    ...
    *,
    ...
    return ReportLlmJobConfig(
        kind=operation,
        queue=queue,
        retry=True if retry is None else str(retry).strip().lower() not in {"0", "false", "no"},
...
def _run_report_llm_reimport_job(job_id: str) -> bool:
    ...
    try:
        ...
        error_detail = (
            "Raw report source could not be materialized from storage. "
            f"{exc}"
        )
        _mark_report_upload_jobs_lost(pdf, error_detail)
        job.mark_lost(error_detail)
...
def _run_report_llm_import_job(job_id: str) -> bool:
    ...
    try:
        ...
        error_detail = (
            "Stored report source could not be materialized from storage. "
            f"{exc}"
        )
        upload_job.mark_lost(error_detail)
        job.mark_lost(error_detail)
