...
fn stable_file_identity_impl(
    ...
    let current_path = FileMetadataIdentity::from_metadata(&std::fs::metadata(&path)?);
...
#[cfg(test)]
mod tests {
    use super::{stable_file_identity_impl, stable_snapshot_to_path_impl, DEFAULT_CHUNK_SIZE};
    use sha2::{Digest, Sha256};
    use std::fs;
